...
public partial class WebSettingsWindow : Window
{
    ...
    private readonly VdsIntegrationService _vdsService = new();
    private VdsSystemSnapshot _vdsSystem = new(VdsServiceState.Unknown, false, false, null);
    private IReadOnlyList<VdsControllerRow> _vdsControllers = [];
    private VdsAudioBufferState? _vdsAudioBuffer;
    private string? _vdsError;
...
private async Task HandleCommandAsync(WebCommand command)
    {
        switch (command.Action)
        {
            case "ready":
                _webReady = true;
                await RefreshVdsAsync(showToast: false);
            ...
            case "resetOsd":
                ...
                break;
            case "refreshVds":
                await RefreshVdsAsync(showToast: true);
                PostState(command.Tab, silent: true);
                break;
            case "openVdsBluetooth":
                _vdsService.OpenBluetoothSettings();
                break;
            case "startVdsService":
                await _vdsService.StartServiceAsync();
                await RefreshVdsAsync(showToast: false);
                PostState(command.Tab, silent: true);
                PostToast("vDS 服务已启动", "success");
                break;
            case "attachVds":
                await AttachVdsAsync(command);
                break;
            case "detachVds":
                await DetachVdsAsync(command);
                break;
            case "applyVdsBuffer":
                await SetVdsBufferAsync(command.Chunks ?? VdsIntegrationService.DefaultAudioBufferChunks, command.Tab);
                break;
            case "resetVdsBuffer":
                await SetVdsBufferAsync(VdsIntegrationService.DefaultAudioBufferChunks, command.Tab);
                break;
            case "save":
                SaveSettings();
                PostState(command.Tab, silent: true);
    ...
    private async Task RefreshVdsAsync(bool showToast)
    {
        _vdsSystem = _vdsService.GetSystemSnapshot();
        _vdsError = null;
        if (!_vdsSystem.VdsctlAvailable)
        {
            _vdsControllers = [];
            _vdsAudioBuffer = null;
            _vdsError = "找不到 vdsctl.exe，请先安装 vDS 或配置 VDS_ROOT。";
            if (showToast)
            {
                PostToast(_vdsError, "error");
            }
            return;
        }
        ...
        try
        {
            _vdsControllers = await _vdsService.GetControllersAsync(_vdsSystem.VdsctlPath!);
            _vdsAudioBuffer = await _vdsService.GetAudioBufferAsync(_vdsSystem.VdsctlPath!);
            if (showToast)
            {
                PostToast("vDS 状态已刷新", "success");
            }
        }
        catch (Exception ex)
        {
            _vdsError = ex.Message;
            if (showToast)
            {
                PostToast($"刷新 vDS 状态失败：{ex.Message}", "error");
            }
        }
    }

    private async Task AttachVdsAsync(WebCommand command)
    {
        if (string.IsNullOrWhiteSpace(command.Address))
        {
            throw new InvalidOperationException("请先在手柄列表中选择要注册的手柄。");
        }
        if (!_vdsSystem.VdsctlAvailable)
        {
            throw new InvalidOperationException("找不到 vdsctl.exe，请先安装 vDS 或配置 VDS_ROOT。");
        }

        await _vdsService.AttachAsync(
            _vdsSystem.VdsctlPath!,
            command.Address,
            string.IsNullOrWhiteSpace(command.Profile) ? "auto" : command.Profile,
            string.IsNullOrWhiteSpace(command.Port) ? "auto" : command.Port);
        await RefreshVdsAsync(showToast: false);
        PostState(command.Tab, silent: true);
        PostToast("手柄已注册并连接", "success");
    }

    private async Task DetachVdsAsync(WebCommand command)
    {
        if (string.IsNullOrWhiteSpace(command.Address))
        {
            throw new InvalidOperationException("请先在手柄列表中选择要取消注册的手柄。");
        }
        if (!_vdsSystem.VdsctlAvailable)
        {
            throw new InvalidOperationException("找不到 vdsctl.exe，请先安装 vDS 或配置 VDS_ROOT。");
        ...
        await _vdsService.DetachAsync(_vdsSystem.VdsctlPath!, command.Address);
        await RefreshVdsAsync(showToast: false);
        PostState(command.Tab, silent: true);
        PostToast("手柄注册已取消", "success");
    }

    private async Task SetVdsBufferAsync(int chunks, string? tab)
    {
        if (!_vdsSystem.VdsctlAvailable)
        {
            throw new InvalidOperationException("找不到 vdsctl.exe，请先安装 vDS 或配置 VDS_ROOT。");
        ...
        var reply = await _vdsService.SetAudioBufferAsync(_vdsSystem.VdsctlPath!, chunks);
        _vdsAudioBuffer = reply;
        _vdsError = null;
        PostState(tab, silent: true);
        PostToast($"音频 / 震动缓冲已设置为 {reply.Chunks} 块（{reply.Milliseconds} ms）", "success");
    }
...
vds = new
        {
            available = _vdsSystem.VdsctlAvailable,
            serviceState = _vdsSystem.ServiceState.ToString(),
            serviceStatus = GetVdsServiceStatus(_vdsSystem.ServiceState),
            usbipInstalled = _vdsSystem.UsbipInstalled,
            hidHideInstalled = _vdsSystem.HidHideInstalled,
            vdsctlPath = _vdsSystem.VdsctlPath,
            error = _vdsError,
            controllers = _vdsControllers.Select(controller => new
            {
                name = controller.Name,
                address = controller.Address,
                connection = controller.ConnectionDisplay,
                serial = string.IsNullOrWhiteSpace(controller.Serial) ? "—" : controller.Serial,
                bluetoothAddress = string.IsNullOrWhiteSpace(controller.MacAddress) ? controller.Address : controller.MacAddress,
                buildTime = string.IsNullOrWhiteSpace(controller.BuildTime) ? "—" : controller.BuildTime,
                board = string.IsNullOrWhiteSpace(controller.Board) ? "—" : controller.Board,
                firmware = string.IsNullOrWhiteSpace(controller.Firmware) ? "—" : controller.Firmware,
                color = string.IsNullOrWhiteSpace(controller.ColorName) ? "—" : controller.ColorName,
                registered = controller.RegistrationDisplay,
                virtualUsb = controller.VirtualUsbDisplay,
                profile = controller.ProfileDisplay,
                online = controller.Online,
                connected = controller.Connected
            }).ToArray(),
            audioBuffer = _vdsAudioBuffer is null
                ? null
                : new
                {
                    chunks = _vdsAudioBuffer.Chunks,
                    milliseconds = _vdsAudioBuffer.Milliseconds
                }
        },        theme = new
        {
...
private static string GetVdsServiceStatus(VdsServiceState state) => state switch
    {
